...
func configureAPI(api *operations.SportAPI) http.Handler {
	...
	api.MonitorDeleteFileHandler = monitor.DeleteFileHandlerFunc(func(params monitor.DeleteFileParams, principal *v1.Principal) middleware.Responder {
		return server.DeleteFile(params, principal)
	})
	...
	api.MonitorDownloadFileHandler = monitor.DownloadFileHandlerFunc(func(params monitor.DownloadFileParams) middleware.Responder {
		return server.DownloadFile(params)
	})
	...
	api.MonitorGetClubFileZipHandler = monitor.GetClubFileZipHandlerFunc(func(params monitor.GetClubFileZipParams, principal *v1.Principal) middleware.Responder {
		return server.GetClubFileZip(params, principal)
	})
	...
	api.MonitorUploadFileHandler = monitor.UploadFileHandlerFunc(func(params monitor.UploadFileParams, principal *v1.Principal) middleware.Responder {
		return server.UploadFile(params, principal)
	})
